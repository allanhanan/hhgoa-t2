...
### Figure 1: Latency Curve vs `QA_NULL_THRESHOLD`
*(Generated by `tune_thresholds.py` — sweeps no-answer rejection floor vs fast-exit latency)*
```text
+-- LATENCY CURVE VS QA_NULL_THRESHOLD -----------+
1016.1 | o
 914.9 |
 813.6 |
 712.4 |
 ...
 105.2 |
   4.0 |     o   o   *   o   o   o
       +------------------------------
         ...
         ^ QA_NULL_THRESHOLD (* = Lowest Latency: 3.97 ms at 0.25)
+----------------------------------------------------------+
```

### Figure 2: Latency Curve vs `ANN_TOP_K` (Candidate Rescore Pool)
*(Generated by `tune_thresholds.py` — sweeps candidate pool size vs float16 dot-product time)*
```text
+-- LATENCY CURVE VS ANN_TOP_K CANDIDATES ----------+
   9.9 |                 o
   9.3 |                 |
   8.8 |                 |
   8.2 |                 |
   ...
   4.2 | o   *
       +----------------------
            4     8    16    32    64
         ^ ANN_TOP_K (* = Lowest Latency: 4.23 ms at 8)
+----------------------------------------------------+
```
